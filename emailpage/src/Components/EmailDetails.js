...
: ''; 
  ...
  return (
    <div className="border relative custom-border bg-white flex-1 rounded-lg  m-4 w-[40rem]">
      <button onClick={()=>handleFavorite(emailBodyDetails?.id)} className="custom-accent  relative top-10 left-[25rem] text-white rounded-l-full rounded-r-full p-1 px-3">{favoriteEmail.includes(emailBodyDetails?.id)?"Remove from favorites":"Mark as Favourite"}</button>
       <div className="flex ">
